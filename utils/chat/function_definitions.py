...
def get_function_tool_def(
    ...
    *,
    ...
    """
    meta = func_metadata(func, skip_names=skip_names, structured_output=None)
    try:
        params_schema = meta.arg_model.model_json_schema(
            by_alias=True,
            schema_generator=StrictJsonSchema,  # raises on warnings to catch non-serializable types
        )
    except Exception:
        # Fall back to standard generator if strict schema generation fails
        params_schema = meta.arg_model.model_json_schema(by_alias=True)

    # Ensure JSON object input with no extra fields
