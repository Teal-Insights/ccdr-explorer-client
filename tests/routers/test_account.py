...
def test_refresh_token_endpoint(auth_client: TestClient, test_account: Account):
    ...
    response = auth_client.post(
        app.url_path_for("refresh_token"), follow_redirects=False
    )
    assert response.status_code == 303
...
def test_password_reset_flow(
    ...
    assert call_args["from"] == "noreply@ccdrexplorer.com"
...
def test_logout_endpoint(auth_client: TestClient):
    response = auth_client.get(app.url_path_for("logout"), follow_redirects=False)
    assert response.status_code == 303
...
def test_request_email_update_success(
    auth_client: TestClient, test_account: Account, mock_resend_send
):
    ...
    assert response.status_code == 303
    assert (
        f"{app.url_path_for('read_profile')}?email_update_requested=true"
...
def test_request_email_update_already_registered(
    ...
    existing_account = Account(
        email=existing_email, hashed_password=get_password_hash("Test123!@#")
    )
...
def test_confirm_email_update_success(
    ...
    assert response.status_code == 303
    assert (
        f"{app.url_path_for('read_profile')}?email_updated=true"
        in response.headers["location"]
    )
...
def test_confirm_email_update_invalid_token(
    unauth_client: TestClient, session: Session, test_account: Account
):
